...
map key command: Maps a key to a Vimium command. Overrides Chrome's default behavior for that key, if any.
...
map <c-d> scrollPageDown maps ctrl+d to scrolling the page down. Chrome's default behavior of showing a bookmark dialog is suppressed.
map r reload hard maps the r key to reloading the page, and also includes the "hard" option to hard-reload the page.
...
Vimium will work on new tab pages which are opened with Vimium's createTab command (mapped to t by default).
To have Vimium work on all new tab pages opened by the browser (e.g. via cmd-t or ctrl-t shortcuts), a companion Vimium New Tab Page extension is required.
Once that is installed, all new tabs will open a blank Vimium new tab page.
More documentation
FAQ
Command listing
Vimium's GitHub wiki: documentation for the more advanced features.
Contributing
